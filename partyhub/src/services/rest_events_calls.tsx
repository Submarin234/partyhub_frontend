...
const getEventById = async  (req: Request,res: Response) => {
    ...
    let result: AxiosResponse = await axios.get(`${BASE_URL}/event/find/id/${eventId}`);
...
const getEventByName = async  (req: Request,res: Response) => {
    ...
    let result: AxiosResponse = await axios.get(`${BASE_URL}/event/find/${eventName}`);
    let event: Event = result.data;
    return res.status(200).json({
        message: event
    });
...
const updateEvent = async (req: Request, res: Response) => {
    ...
    let response: AxiosResponse = await axios.put(`${BASE_URL}/event/${eventId}`, {
        ...(eventName && { eventName })
    });
    return res.status(200).json({
        message: response.data
...
const deleteEvent = async (req: Request, res: Response) => {
    let eventId: number = Number(req.params.eventId);
    let response: AxiosResponse = await axios.delete(`${BASE_URL}/event/${eventId}`);
    return res.status(200).json({
        message: response.data
    });
};

export default {getAllEvents,getEventById,getEventByName,addEvent,updateEvent,deleteEvent};